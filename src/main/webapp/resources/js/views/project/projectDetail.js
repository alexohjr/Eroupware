...
// 프로젝트 수정 유효성 검사
function validateModifyProjectForm() {
	var form = $(".projectModifyForm");
	
	form.submit(function() {
		var selectedMemberLength = $(".selectedMemberSpan").length;
		
		if(!$(".inputProjectArea .inputTitle").val().trim()) {
			alert("프로젝트 제목을 입력해주세요.");
			$(".inputProjectArea .inputTitle").focus();
			return false;
		} else if(!$(".inputProjectArea .inputContent").val().trim()) {
			alert("프로젝트 내용을 입력해주세요.");
			$(".inputProjectArea .inputContent").focus();
			return false;
